...
then somewhere something is going to have to implement the stree interface for backing store.
but that's not this.

so it goes

local block device -> storage mechanism (stree_v) -> backingstore (file/block device/etc)

this isn't the best place for this (in stree_v) but there's no general library for
*/
package zosbd2_stree_v_storage_mechanism
...
type Zosbd2_stree_v_storage_mechanism struct {
	log           *tools.Nixomosetools_logger
	rawstore      *stree_v_lib.Stree_v // the zosbd2cmd backing store is an stree_v that was already injected with a file/memory backing store
	data_pipeline *list.List
}

var _ zosbd2interfaces.Storage_mechanism = &Zosbd2_stree_v_storage_mechanism{}
var _ zosbd2interfaces.Storage_mechanism = (*Zosbd2_stree_v_storage_mechanism)(nil)

func New_zosbd2_storage_mechanism(log *tools.Nixomosetools_logger, stree *stree_v_lib.Stree_v,
	data_pipeline *list.List) *Zosbd2_stree_v_storage_mechanism {
	var z Zosbd2_stree_v_storage_mechanism
	z.log = log
	z.rawstore = stree
...
func /* (this *Zosbd2_stree_v_storage_mechanism) */ Generate_key_from_block_num(block_num uint64) string {
...
func (this *Zosbd2_stree_v_storage_mechanism) Discard_block(start_in_bytes uint64, length uint32) tools.Ret {
	...
	if discard_range_start == -1 || discard_range_end == -1 {
		...
		/* funny that we never actually called delete...
		   So the zos version of this that I got this from was able to to a sweeping delete in one
			 call, but stree_v doesn't have that so we just loop through the blocks anyway. maybe
...
func (this *Zosbd2_stree_v_storage_mechanism) Get_block_size() uint32 {
